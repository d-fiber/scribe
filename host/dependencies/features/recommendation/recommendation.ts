// Copyright (C) 2026 Fiber
//
// This file is part of scribe and is made available under the PolyForm Shield
// License 1.0.0. The full terms are in the LICENSE file at the root of this
// repository, and at https://polyformproject.org/licenses/shield/1.0.0
//
// What you may do:
// - Use this software for any purpose, including commercially, and build and
//   sell your own products on top of it.
// - Change it, and create new works based on it.
// - Distribute copies of it, with or without your changes.
//
// The one thing you may not do:
// - Use it to provide any product that competes with scribe, or with any
//   product Fiber or its affiliates provide using scribe. Products compete
//   even when they are offered free of charge, through a different kind of
//   interface, or for a different technical platform.
//
// If you pass this software on:
// - Anyone who receives any part of it from you must also receive these terms,
//   or the URL above, together with the "Required Notice" line carried by the
//   LICENSE file.
//
// Disclaimer:
// AS FAR AS THE LAW ALLOWS, THIS SOFTWARE COMES AS IS, WITHOUT ANY WARRANTY OR
// CONDITION, AND THE LICENSOR WILL NOT BE LIABLE TO YOU FOR ANY DAMAGES ARISING
// OUT OF THESE TERMS OR THE USE OR NATURE OF THE SOFTWARE, UNDER ANY KIND OF
// LEGAL CLAIM.
//
// This header is a summary written for convenience. Where it differs from the
// LICENSE file, the LICENSE file governs.

import type { Result } from "@scribe/core/contracts/result.ts";

export interface GorseEntity {
  readonly id: string;
  readonly labels?: Record<string, unknown>;
  readonly comment?: string;
}

export interface GorseUser extends GorseEntity {}

export interface GorseItem extends GorseEntity {
  readonly categories?: string[];
  readonly isHidden?: boolean;
  readonly timestamp?: string;
}

export interface GorseFeedback {
  readonly type: string;
  readonly userId: string;
  readonly itemId: string;
  readonly value?: number;
  readonly timestamp?: string;
}

export interface RecommendationOptions {
  readonly count?: number;
  readonly category?: string;
  readonly offset?: number;
}

export enum RecommendationUpsertUserError {
  Unexpected = "unexpected",
}

export enum RecommendationUpsertItemError {
  Unexpected = "unexpected",
}

export enum RecommendationDeleteUserError {
  NotFound = "not_found",
  Unexpected = "unexpected",
}

export enum RecommendationDeleteItemError {
  NotFound = "not_found",
  Unexpected = "unexpected",
}

export enum RecommendationFeedbackError {
  Unexpected = "unexpected",
}

export enum RecommendationRecommendError {
  Unexpected = "unexpected",
}

export interface RecommendationService {
  upsertUser(
    user: GorseUser,
  ): Promise<Result<void, RecommendationUpsertUserError>>;
  upsertItem(
    item: GorseItem,
  ): Promise<Result<void, RecommendationUpsertItemError>>;
  deleteUser(
    userId: string,
  ): Promise<Result<void, RecommendationDeleteUserError>>;
  deleteItem(
    itemId: string,
  ): Promise<Result<void, RecommendationDeleteItemError>>;
  insertFeedback(
    feedback: GorseFeedback[],
  ): Promise<Result<void, RecommendationFeedbackError>>;
  recommend(
    userId: string,
    options?: RecommendationOptions,
  ): Promise<Result<string[], RecommendationRecommendError>>;
}

export { RecommendationClient } from "@scribe/host/dependencies/features/recommendation/recommendation.impl.ts";
