...
Deno.test("purpose: a password-reset token is refused by a sign-in reader", async () => {
  const token = await forgeToken("+33612345678", AccountRole.User, { purpose: PendingTokenPurpose.PasswordReset });
...
Deno.test("purpose: a sign-in token is refused by a password-reset reader", async () => {
  const token = await forgeToken("u1@example.com", AccountRole.User, { purpose: PendingTokenPurpose.SignIn });
...
Deno.test("purpose: the role check still applies within one purpose", async () => {
  const token = await forgeToken("a1@example.com", AccountRole.Admin, { purpose: PendingTokenPurpose.PasswordReset });
  const payload = await reset.payload(token);

  assertEquals(payload?.role, AccountRole.Admin);
  assertNotEquals(payload?.role, AccountRole.User);
});
