...
import {
  ...
  PhoneCheckStatus,
} from "@scribe/host/dependencies/security/auth/src/_core/validator.ts";
...
Deno.test("profile: age bounds", () => {
  ...
  assertEquals(AuthValidator.profile.isValidBirthday(yearsAgo(17)), false);
  assertEquals(AuthValidator.profile.isValidBirthday(yearsAgo(18)), true);
  assertEquals(AuthValidator.profile.isValidBirthday(yearsAgo(99)), true);
  assertEquals(AuthValidator.profile.isValidBirthday(yearsAgo(100)), false);
  assertEquals(AuthValidator.profile.isValidBirthday(Date.now() + 86400000), false);
});

Deno.test("profile: name length", () => {
  assertEquals(AuthValidator.profile.isValidName("a"), false);
  assertEquals(AuthValidator.profile.isValidName("ab"), true);
  assertEquals(AuthValidator.profile.isValidName("a".repeat(120)), true);
  assertEquals(AuthValidator.profile.isValidName("a".repeat(121)), false);
});
