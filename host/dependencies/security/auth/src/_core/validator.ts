...
import { enumValues, Gender } from "@scribe/core/contracts/enums.ts";
...
class ProfileValidator {
  ...
  static isValidGender(gender: string): gender is Gender {
    return enumValues(Gender).includes(gender as Gender);
  }

  static isValidBirthday(birthday: number): boolean {
    ...
    return birthDate < now && age >= 18 && age <= 99;
  }
}
...
export class AuthValidator {
  ...
  static readonly profile = ProfileValidator;
}
