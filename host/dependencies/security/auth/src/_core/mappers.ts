...
import type { GoTrueSessionResponse, GoTrueUser } from "@scribe/host/dependencies/security/auth/src/_core/gotrue/transport.ts";
import { AccountRole, type Session, type SessionUser } from "@scribe/core/contracts/account.ts";
import { fromBase64Url, jsonFromBase64Url } from "@scribe/core/runtime/support/crypto/base64url.ts";
import { Env } from "@scribe/host/env.ts";

class AccountMapper {
  static role(raw: GoTrueUser | GoTrueSessionResponse): AccountRole {
    const user: GoTrueUser | undefined = "app_metadata" in raw ? raw : raw.user;
    return user?.app_metadata.role === AccountRole.Admin ? AccountRole.Admin : AccountRole.User;
  }
...
class JwtMapper {
  ...
  static async account(
    ...
    try {
      ...
      return {
        userId,
        role: AccountMapper.role(claims as unknown as GoTrueUser),
      };
    } catch {
      return null;
    }
