...
// Forge a pending token the way PendingToken.issue() signs one, but without the
// database round-trip: most endpoint tests need the raw token *before* they can
// build the rest mock seed that carries its hash. `pending_token_forge.test.ts`
// pins this forge against the real implementation, so a change to the signing
// format fails there instead of silently making every fixture unverifiable.

import { type PendingToken, PendingTokenPurpose } from "@scribe/host/dependencies/security/auth/src/_core/pending_token.ts";
