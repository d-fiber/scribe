...
Deno.test("scope: a user only reads its own rows", async () => {
  const mock = installRestMock({ internal_t__app_user_settings: [...SETTINGS] });
  try {
    const rows = await withIdentity(USER, () => rest.internal_t__app_user_settings().get());
...
Deno.test("scope: an admin sees a whole user table", async () => {
  const mock = installRestMock({ internal_t__app_user_settings: [...SETTINGS] });
  try {
    const rows = await withIdentity(ADMIN, () => rest.internal_t__app_user_settings().get());
...
Deno.test("scope: an admin is bounded on an admin table", async () => {
  ...
  try {
    const rows = await withIdentity(ADMIN, () => rest.internal_t__admin_users_settings().get());
    assertEquals(rows.length, 1);
...
Deno.test("scope: without an identity, service scope (no filter)", async () => {
  const mock = installRestMock({ internal_t__app_user_settings: [...SETTINGS] });
  try {
    const rows = await withIdentity(null, () => rest.internal_t__app_user_settings().get());
    assertEquals(rows.length, 2);
  } finally {
    mock.restore();
  }
  ...
  async fn() {
    ...
    try {
      const ok = await withIdentity(
        USER,
        () => rest.internal_t__app_user_settings().update({ theme_mode: DeviceThemeMode.SYSTEM }),
      );
...
Deno.test("scope: an update without filter or identity is refused", async () => {
  ...
  try {
    const ok = await withIdentity(
      null,
      () => rest.internal_t__app_user_settings().update({ theme_mode: DeviceThemeMode.DARK }),
    );
...
Deno.test("scope: .unscoped() returns the whole table to a user", async () => {
  ...
  try {
    const rows = await withIdentity(
      USER,
      () => rest.internal_t__app_user_settings().unscoped().get(),
    );
    assertEquals(rows.length, 2);
  } finally {
    mock.restore();
...
Deno.test({
  ...
  sanitizeResources: false,
  async fn() {
    const mock = installRestMock({ internal_t__app_user_settings: [] });
    try {
      const ok = await withIdentity(
        USER,
        () => rest.internal_t__app_user_settings().insert({ localization: "fr" } as never),
      );
      assertEquals(ok, true);
