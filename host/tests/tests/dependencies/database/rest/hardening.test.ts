...
const templates = () => rest.internal_t__email_templates();
...
Deno.test("cross-owner: a user cannot sweep an admin-owned table", async () => {
  ...
  try {
    await assertRejects(
      () => withIdentity(USER, () => rest.internal_t__admin_users_devices().get()),
      OwnerScopeError,
    );
  } finally {
    mock.restore();
...
Deno.test("cross-owner: an explicit predicate on the owner column is allowed", async () => {
  ...
  try {
    const rows = await withIdentity(
      USER,
      () =>
        rest.internal_t__admin_users_devices()
...
Deno.test("cross-owner: .unscoped() stays the deliberate way through", async () => {
  ...
  try {
    const rows = await withIdentity(
      USER,
      () => rest.internal_t__admin_users_devices().unscoped().get(),
    );
...
Deno.test("unbounded write: an unowned table is no longer a free-for-all", async () => {
  const mock = installRestMock({ internal_t__email_templates: [...TEMPLATES] });
  try {
...
Deno.test("insert: an explicit null owner is filled in, not written", async () => {
  ...
  try {
    await withIdentity(
      USER,
      () => rest.internal_t__app_user_settings().insert({ user_id: null } as never),
    );
