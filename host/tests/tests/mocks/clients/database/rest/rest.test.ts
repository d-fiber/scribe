...
import { createRestMock } from "@scribe/host/tests/mocks/dependencies/database/rest/rest.ts";
...
Deno.test(
  ...
  async () => {
    const mock = createRestMock({ widgets: [] });
...
Deno.test(
  "RestMock: a generated table method reads through the fake db",
  async () => {
    const mock = createRestMock({
