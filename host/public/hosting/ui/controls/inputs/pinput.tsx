...
export interface AppPinputProps extends AppFieldProps {
  length?: number;
}

const defaultLength = 6;

export function makeAppPinput(colors: AppColors, fonts: AppFonts) {
  const { Field, fieldStyle, fieldAttributes } = makeField(colors, fonts);

  function AppPinput(props: AppPinputProps) {
    const {
      label,
      requirement,
      size = "large",
    ...
    return (
      <Field label={label} requirement={requirement}>
        <input
          ...
          style={fieldStyle({ size, shape, opaque, centered: true })}
          type="text"
          inputMode="numeric"
          autoComplete={autoComplete ?? "one-time-code"}
          maxLength={length}
          minLength={length}
          pattern={pattern ?? `\\d{${length}}`}
        />
      </Field>
    );
  }
