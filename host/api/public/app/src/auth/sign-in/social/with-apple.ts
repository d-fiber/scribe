...
import { SocialSignInError } from "@scribe/host/dependencies/security/auth/src/sign_in/sign_in.ts";
import { Time } from "@scribe/core/contracts/common/time.ts";
import { ApiContext, ApiEndpoint, Caller, Required } from "@scribe/core/kernel/endpoint/api.ts";

export class SignInWithAppleEndpoint extends ApiEndpoint {
  ...
  protected async run(ctx: ApiContext): Promise<Response> {
    ...
    const result = await clients.security.auth.signIn.user.social.apple.withIdToken(
      body.id_token,
      body.nonce,
      body.access_token,
    );
    ...
    if (!result.ok) {
      switch (result.error) {
        case SocialSignInError.SignInFailed:
          return this.response.unauthorized({
            message: "We could not sign you in with Apple. Please try again.",
          });
        case SocialSignInError.TooManyRequests:
