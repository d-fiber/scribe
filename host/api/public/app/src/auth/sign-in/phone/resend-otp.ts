...
export class ResendPhoneOtpEndpoint extends ApiEndpoint {
  ...
  protected async run(ctx: ApiContext): Promise<Response> {
    ...
    const result = await clients.security.auth.signIn.user.phone.resend(body.token);

    if (!result.ok) {
      switch (result.error) {
        case ResendError.InvalidOrExpiredToken:
          return this.response.unauthorized({
            message: "Your session has expired or is invalid. Please sign in again to request a new verification code.",
          });
