...
import { PhoneSignInError } from "@scribe/host/dependencies/security/auth/src/sign_in/sign_in.ts";
import { Time } from "@scribe/core/contracts/common/time.ts";
import { ApiContext, ApiEndpoint, Caller, Required } from "@scribe/core/kernel/endpoint/api.ts";

export class SignInWithPhoneEndpoint extends ApiEndpoint {
  ...
  protected async run(ctx: ApiContext): Promise<Response> {
    ...
    const result = await clients.security.auth.signIn.user.phone.withPhoneAndPassword(
      body.phone,
      body.password,
    );
    ...
    if (!result.ok) {
      switch (result.error) {
        case PhoneSignInError.PhoneRequired:
        case PhoneSignInError.PasswordRequired:
          return this.response.badRequest();
        case PhoneSignInError.InvalidCredentials:
          return this.response.unauthorized(
            { message: "The phone number or password you entered is incorrect." },
          );
        case PhoneSignInError.PhoneNotConfirmed:
          return this.response.forbidden({
            message: "Your phone number has not been verified yet. We have sent you a new verification code by SMS.",
          });
        case PhoneSignInError.TooManyRequests:
