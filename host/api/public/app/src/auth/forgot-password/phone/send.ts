...
export class SendPhoneResetPasswordEndpoint extends ApiEndpoint {
  ...
  protected async run(ctx: ApiContext): Promise<Response> {
    ...
    const result = await clients.security.auth.resetPassword.user.phone.send(body.phone);
    ...
    return this.response.ok({
      message:
        "If an account is associated with this phone number, you will receive a verification code by SMS shortly.",
    });
  }
}
