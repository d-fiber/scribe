...
import { VerifyPhoneResetOtpError } from "@scribe/host/dependencies/security/auth/src/reset_password/reset_password.ts";
import { Time } from "@scribe/core/contracts/common/time.ts";
import { ApiContext, ApiEndpoint, Caller, Required } from "@scribe/core/kernel/endpoint/api.ts";

export class VerifyPhoneResetOtpEndpoint extends ApiEndpoint {
  ...
  protected async run(ctx: ApiContext): Promise<Response> {
    ...
    const result = await clients.security.auth.resetPassword.user.phone.verify(
      body.phone,
      body.otp,
    );
    ...
    return this.response.ok({ data: { token: result.data.resetToken } });
  }
}
