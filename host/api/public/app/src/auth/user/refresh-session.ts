...
import { RefreshSessionError } from "@scribe/host/dependencies/security/auth/src/session/session.ts";
...
export class RefreshSessionEndpoint extends ApiEndpoint {
  ...
  protected async run(ctx: ApiContext): Promise<Response> {
    const body = ctx.body({ refresh_token: Required(String) });
    if (!body) return this.response.badRequest();

    const result = await clients.security.auth.session.refresh(body.refresh_token);

    if (!result.ok) {
      switch (result.error) {
        case RefreshSessionError.Unauthorized:
          return this.response.unauthorized();
        case RefreshSessionError.TooManyRequests:
