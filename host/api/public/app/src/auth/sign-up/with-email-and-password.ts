...
import { EmailSignUpError } from "@scribe/host/dependencies/security/auth/src/sign_up/sign_up.ts";
...
export class SignUpEndpoint extends ApiEndpoint {
  ...
  protected async run(ctx: ApiContext): Promise<Response> {
    ...
    const result = await clients.security.auth.signUp.user.email.withEmailAndPassword({
      email: body.email,
    ...
    if (!result.ok) {
      ...
      switch (result.error) {
        ...
        case EmailSignUpError.TooManyRequests:
