...
export class SignUpWithGoogleEndpoint extends ApiEndpoint {
  ...
  protected async run(ctx: ApiContext): Promise<Response> {
    ...
    const result = await clients.security.auth.signUp.user.social.google.withIdToken({
      ...
      switch (result.error) {
        case SocialSignUpError.AccountAlreadyExists:
          return this.response.conflict({
            ...
            message: "An account already exists for this Google identity. Sign in instead.",
          });
