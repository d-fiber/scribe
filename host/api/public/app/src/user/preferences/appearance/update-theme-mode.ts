...
import { DeviceThemeMode, enumValues } from "@scribe/core/contracts/enums.ts";
...
export class UpdateThemeModeEndpoint extends ApiEndpoint {
  ...
  protected async run(ctx: ApiContext): Promise<Response> {
    ...
    const body = ctx.body({ theme_mode: Required(String) });
    if (!body) return this.response.badRequest();

    if (!enumValues(DeviceThemeMode).includes(body.theme_mode as DeviceThemeMode)) {
    ...
    const ok = await rest
      ...
      .update({ theme_mode: body.theme_mode as DeviceThemeMode });
