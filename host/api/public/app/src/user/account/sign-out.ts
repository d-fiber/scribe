  ...
  protected async run(_ctx: ApiContext): Promise<Response> {
    ...
    if (!result.ok) {
      switch (result.error) {
        ...
        default:
          // Sign-out is best-effort by design the client should never
          // get stuck unable to sign out over a server-side quirk but
          // an unrecognised error was silently treated as success with no
          // trace. Kept the ok() (still intentional), added the log.
