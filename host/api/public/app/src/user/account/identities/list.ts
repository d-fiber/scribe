...
import { IdentitiesError } from "@scribe/host/dependencies/security/auth/src/user/user.ts";
import { Time } from "@scribe/core/contracts/common/time.ts";
import { ApiContext, ApiEndpoint, Caller } from "@scribe/core/kernel/endpoint/api.ts";

export class ListIdentitiesEndpoint extends ApiEndpoint {
  ...
  protected async run(_ctx: ApiContext): Promise<Response> {
    const result = await clients.security.auth.user.identities.list();

    if (!result.ok) {
      switch (result.error) {
        case IdentitiesError.Unauthorized:
          return this.response.unauthorized();
        case IdentitiesError.TooManyRequests:
