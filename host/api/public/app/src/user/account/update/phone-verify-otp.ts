...
import { ConfirmUserPhoneError } from "@scribe/host/dependencies/security/auth/src/user/phone.ts";
import { Time } from "@scribe/core/contracts/common/time.ts";
import { ApiContext, ApiEndpoint, Caller, Required } from "@scribe/core/kernel/endpoint/api.ts";

export class ConfirmPhoneChangeEndpoint extends ApiEndpoint {
  ...
  protected async run(ctx: ApiContext): Promise<Response> {
    ...
    const body = ctx.body({ phone: Required(String), otp: Required(String) });
    if (!body) return this.response.badRequest();

    const result = await clients.security.auth.user.phone.confirmChange(
      userId,
      body.phone,
      body.otp,
    );
