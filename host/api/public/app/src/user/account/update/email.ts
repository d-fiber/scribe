...
import { UpdateUserEmailError } from "@scribe/host/dependencies/security/auth/src/user/user.ts";
import { Time } from "@scribe/core/contracts/common/time.ts";
import { ApiContext, ApiEndpoint, Caller, Required } from "@scribe/core/kernel/endpoint/api.ts";

export class UpdateEmailEndpoint extends ApiEndpoint {
  ...
  protected async run(ctx: ApiContext): Promise<Response> {
    ...
    const result = await clients.security.auth.user.email.update(
      userId,
      body.email,
    );
    ...
    return this.response.ok({
      message: "We sent a confirmation link to your new email address. It becomes active once confirmed.",
    });
  }
}
