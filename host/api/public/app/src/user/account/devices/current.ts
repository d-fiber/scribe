...
export class CurrentDeviceEndpoint extends ApiEndpoint {
  ...
  protected async run(_ctx: ApiContext): Promise<Response> {
    const device = await clients.security.auth.session.device.current();
    if (!device) return this.response.notFound();

    return this.response.ok({ data: device });
  }
}
