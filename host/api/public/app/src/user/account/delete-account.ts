...
import { DeleteAccountError } from "@scribe/host/dependencies/security/auth/src/session/session.ts";
...
export class DeleteAccountEndpoint extends ApiEndpoint {
  ...
  protected async run(_ctx: ApiContext): Promise<Response> {
    const result = await clients.security.auth.session.delete();

    if (!result.ok) {
      switch (result.error) {
        case DeleteAccountError.Unauthorized:
          return this.response.unauthorized();
        case DeleteAccountError.TooManyRequests:
