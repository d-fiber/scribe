...
export class RemoveIssueReportEndpoint extends ApiEndpoint {
  ...
  protected async run(ctx: ApiContext): Promise<Response> {
    ...
    const body = ctx.body({ issue_id: Required(String) });
    if (!body) return this.response.badRequest();

    const deleted = await rest
      .internal_t__app_user_issue_reports()
      .where((f) => f.issue_id.eq(body.issue_id))
      .deleteOne((s) => ({ issue_id: s.issue_id }));

    if (!deleted) return this.response.notFound();
