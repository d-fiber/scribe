...
export class UpdateMemberGenderEndpoint extends ApiEndpoint {
  ...
  protected async run(ctx: ApiContext): Promise<Response> {
    const body = ctx.body({ gender: Required(String) });
    if (!body) return this.response.badRequest();

    if (!enumValues(Gender).includes(body.gender as Gender)) {
