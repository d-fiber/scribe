...
export class UpdateMemberBirthdayEndpoint extends ApiEndpoint {
  ...
  protected async run(ctx: ApiContext): Promise<Response> {
    const body = ctx.body({ birthday: Required(Number) });
    if (!body) return this.response.badRequest();

    const birthDate = new Date(body.birthday);
    if (isNaN(birthDate.getTime())) return this.response.badRequest();
    ...
    if (age < 18 || age > 120) return this.response.badRequest();
