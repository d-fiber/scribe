...
export class AdminReadTeamMemberEndpoint extends ApiEndpoint {
  ...
  protected async run(_ctx: ApiContext): Promise<Response> {
    const data = await rest
      ...
      .select((s) => ({
        ...
        admin_users_settings: s.embed("internal_t__admin_users_settings", (settings) => ({
    ...
    if (!p || !s) return this.response.unexpected();
