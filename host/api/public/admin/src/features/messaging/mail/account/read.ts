...
import { AdminSmtpAccountNameEndpoint, payload, READ_RATE_LIMIT } from "./_shared.ts";

export class AdminSmtpAccountReadEndpoint extends AdminSmtpAccountNameEndpoint {
  protected rateLimit() {
  ...
  protected async run(_ctx: ApiContext): Promise<Response> {
    ...
    const result = await clients.features.messagings.mail.accounts.get(
      this.name,
    );
