...
import { AdminMailIdEndpoint, isNotFound, payload, READ_RATE_LIMIT } from "./_shared.ts";

export class AdminMailStatisticsReadEndpoint extends AdminMailIdEndpoint {
  protected rateLimit() {
  ...
  protected async run(_ctx: ApiContext): Promise<Response> {
    ...
    const result = await clients.features.messagings.mail.statistics.get(this.id);
