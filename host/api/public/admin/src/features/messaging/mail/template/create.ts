...
export class AdminEmailTemplateCreateEndpoint extends AdminMailEndpoint {
  protected rateLimit() {
  ...
  protected async run(ctx: ApiContext): Promise<Response> {
    ...
    if (body.html !== undefined && typeof body.html !== "string") {
      return this.invalidField("html", "a string");
    }

    const result = await clients.features.messagings.mail.templates.create({
      name,
      subject,
      text,
