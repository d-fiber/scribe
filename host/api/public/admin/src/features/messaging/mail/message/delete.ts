...
export class AdminMailMessageDeleteEndpoint extends AdminMailIdEndpoint {
  protected rateLimit() {
  ...
  protected async run(_ctx: ApiContext): Promise<Response> {
    ...
    const found = await clients.features.messagings.mail.noreply.get(this.id);
    if (!found.ok) return this.notFoundOr(isNotFound(found.error));

    const result = await clients.features.messagings.mail.noreply.remove(this.id);
