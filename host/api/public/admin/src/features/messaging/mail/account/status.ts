...
import { AdminSmtpAccountNameEndpoint, objectOrNull, WRITE_RATE_LIMIT } from "./_shared.ts";

export class AdminSmtpAccountStatusEndpoint extends AdminSmtpAccountNameEndpoint {
  protected rateLimit() {
  ...
  protected async run(ctx: ApiContext): Promise<Response> {
    ...
    const result = await clients.features.messagings.mail.accounts.setActive(
      this.name,
      body.is_active,
    );
