...
} from "@scribe/host/dependencies/features/messagings/mail/mail.ts";
import { EmailCampaignError } from "@scribe/host/dependencies/features/messagings/mail/mail.ts";
import { CampaignAudience, DeviceOs, enumValues, Localization } from "@scribe/core/contracts/enums.ts";
...
export function isNotFound(error: EmailCampaignError): boolean {
  return error === EmailCampaignError.NotFound;
}

export function isInvalidSchedule(error: EmailCampaignError): boolean {
  return error === EmailCampaignError.InvalidSchedule;
}

export function payload(campaign: EmailCampaign) {
  return {
    id: campaign.id,
    email_template_id: campaign.emailTemplateId,
...
function scheduleJson(schedule: EmailCampaignSchedule) {
...
export function audienceMessage(): string {
  return `one of ${enumValues(CampaignAudience).join(", ")}`;
}

export function audienceOrNull(value: unknown): CampaignAudience | null {
  return enumValues(CampaignAudience).includes(value as CampaignAudience) ? (value as CampaignAudience) : null;
}
...
export function scheduleOrNull(value: unknown): EmailCampaignSchedule | null {
...
const FILTER_KEYS: Record<string, keyof EmailCampaignFilters> = {
  device_os: "deviceOs",
...
export function filtersOrNull(value: unknown): EmailCampaignFilters | null {
  const raw = objectOrNull(value);
  if (!raw) return null;
  ...
  return filters as EmailCampaignFilters;
}
