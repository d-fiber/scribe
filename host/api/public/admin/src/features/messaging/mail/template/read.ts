...
export class AdminEmailTemplateReadEndpoint extends AdminMailIdEndpoint {
  protected rateLimit() {
  ...
  protected async run(_ctx: ApiContext): Promise<Response> {
    ...
    const result = await clients.features.messagings.mail.templates.getById(this.id);
...
export class AdminEmailTemplateReadByNameEndpoint extends AdminMailEndpoint {
  ...
  protected async run(_ctx: ApiContext): Promise<Response> {
    ...
    const result = await clients.features.messagings.mail.templates.getByName(this.#name);
    if (!result.ok) return this.notFoundOr(isNotFound(result.error));
