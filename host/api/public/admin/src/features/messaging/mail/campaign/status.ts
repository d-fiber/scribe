...
import { AdminMailIdEndpoint, isNotFound, objectOrNull, WRITE_RATE_LIMIT } from "./_shared.ts";

export class AdminEmailCampaignStatusEndpoint extends AdminMailIdEndpoint {
  protected rateLimit() {
  ...
  protected async run(ctx: ApiContext): Promise<Response> {
    ...
    const result = await clients.features.messagings.mail.campaigns.setActive(
      this.id,
      body.is_active,
    );
