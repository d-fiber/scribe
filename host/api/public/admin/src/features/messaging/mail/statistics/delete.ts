...
import { AdminMailIdEndpoint, isNotFound, WRITE_RATE_LIMIT } from "./_shared.ts";

export class AdminMailStatisticsDeleteEndpoint extends AdminMailIdEndpoint {
  protected rateLimit() {
  ...
  protected async run(_ctx: ApiContext): Promise<Response> {
    ...
    const found = await clients.features.messagings.mail.statistics.get(this.id);
    if (!found.ok) return this.notFoundOr(isNotFound(found.error));

    const result = await clients.features.messagings.mail.statistics.remove(this.id);
