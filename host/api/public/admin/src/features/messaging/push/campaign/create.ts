...
export class AdminPushCampaignCreateEndpoint extends AdminPushEndpoint {
  protected rateLimit() {
  ...
  protected async run(ctx: ApiContext): Promise<Response> {
    ...
    const result = await clients.features.messagings.notificationPush.campaigns.create({
      pushTemplateId,
      schedule,
      ...(filters ? { filters } : {}),
