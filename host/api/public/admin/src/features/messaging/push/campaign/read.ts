...
import { AdminPushIdEndpoint, isNotFound, payload, READ_RATE_LIMIT } from "./_shared.ts";

export class AdminPushCampaignReadEndpoint extends AdminPushIdEndpoint {
  protected rateLimit() {
  ...
  protected async run(_ctx: ApiContext): Promise<Response> {
    ...
    const result = await clients.features.messagings.notificationPush.campaigns.get(this.id);
