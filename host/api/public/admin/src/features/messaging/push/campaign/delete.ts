...
export class AdminPushCampaignDeleteEndpoint extends AdminPushIdEndpoint {
  protected rateLimit() {
  ...
  protected async run(_ctx: ApiContext): Promise<Response> {
    ...
    const result = await clients.features.messagings.notificationPush.campaigns.remove(this.id);
