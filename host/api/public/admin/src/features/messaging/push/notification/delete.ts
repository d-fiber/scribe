...
export class AdminPushNotificationDeleteEndpoint extends AdminPushIdEndpoint {
  protected rateLimit() {
  ...
  protected async run(_ctx: ApiContext): Promise<Response> {
    ...
    const result = await clients.features.messagings.notificationPush.push.remove(this.id);
