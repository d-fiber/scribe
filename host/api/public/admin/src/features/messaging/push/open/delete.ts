...
export class AdminPushOpenDeleteEndpoint extends AdminPushIdEndpoint {
  protected rateLimit() {
  ...
  protected async run(_ctx: ApiContext): Promise<Response> {
    ...
    const result = await clients.features.messagings.notificationPush.opens.remove(this.id);
