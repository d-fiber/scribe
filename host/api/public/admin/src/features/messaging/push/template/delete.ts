...
import { AdminPushIdEndpoint, isNotFound, WRITE_RATE_LIMIT } from "./_shared.ts";

export class AdminPushTemplateDeleteEndpoint extends AdminPushIdEndpoint {
  protected rateLimit() {
  ...
  protected async run(_ctx: ApiContext): Promise<Response> {
    ...
    const result = await clients.features.messagings.notificationPush.templates.remove(this.id);
