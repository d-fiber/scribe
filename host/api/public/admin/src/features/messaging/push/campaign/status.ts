...
import { AdminPushIdEndpoint, isNotFound, objectOrNull, WRITE_RATE_LIMIT } from "./_shared.ts";

export class AdminPushCampaignStatusEndpoint extends AdminPushIdEndpoint {
  protected rateLimit() {
  ...
  protected async run(ctx: ApiContext): Promise<Response> {
    ...
    const result = await clients.features.messagings.notificationPush.campaigns.setActive(
      this.id,
      body.is_active,
    );
