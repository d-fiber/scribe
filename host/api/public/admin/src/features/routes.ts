...
import { MessagingRouter } from "./messaging/routes.ts";

export class FeaturesRouter extends Router {
  constructor(adminOnly: MiddlewareHandler) {
    super(adminOnly);
  }

  protected routes(app: Hono): void {
    app.route("/devops", DevopsRouter.create());
    app.route("/messaging", MessagingRouter.create());
  }
}
