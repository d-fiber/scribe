...
import { AdminRemoteConfigKeyEndpoint, WRITE_RATE_LIMIT } from "./_shared.ts";

export class AdminRemoteConfigDeleteEndpoint extends AdminRemoteConfigKeyEndpoint {
  protected rateLimit() {
  ...
  protected async run(_ctx: ApiContext): Promise<Response> {
    ...
    const result = await clients.devops.remoteConfigs.config.remove(this.key);
