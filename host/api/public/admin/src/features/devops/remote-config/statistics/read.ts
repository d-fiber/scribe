...
import { AdminRemoteConfigStatisticEndpoint, payload, READ_RATE_LIMIT } from "./_shared.ts";

export class AdminRemoteConfigStatisticsReadEndpoint extends AdminRemoteConfigStatisticEndpoint {
  protected rateLimit() {
  ...
  protected async run(_ctx: ApiContext): Promise<Response> {
    ...
    const result = await clients.devops.remoteConfigs.statistics.get(this.id);
