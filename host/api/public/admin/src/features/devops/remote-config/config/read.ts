...
import { AdminRemoteConfigKeyEndpoint, payload, READ_RATE_LIMIT } from "./_shared.ts";

export class AdminRemoteConfigReadEndpoint extends AdminRemoteConfigKeyEndpoint {
  protected rateLimit() {
  ...
  protected async run(_ctx: ApiContext): Promise<Response> {
    ...
    const result = await clients.devops.remoteConfigs.config.get(this.key);
