...
export class AdminDynamicLinkStatisticsReadEndpoint extends AdminDynamicLinkStatisticEndpoint {
  protected rateLimit() {
  ...
  protected async run(_ctx: ApiContext): Promise<Response> {
    ...
    const result = await clients.devops.dynamicLinks.statistics.get(this.id);
