...
import { AdminDynamicLinkIdEndpoint, payload, READ_RATE_LIMIT } from "./_shared.ts";

export class AdminDynamicLinkReadEndpoint extends AdminDynamicLinkIdEndpoint {
  protected rateLimit() {
  ...
  protected async run(_ctx: ApiContext): Promise<Response> {
    ...
    const result = await clients.devops.dynamicLinks.link.getById(this.id);
