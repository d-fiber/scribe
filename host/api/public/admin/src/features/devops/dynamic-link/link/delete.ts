...
import { AdminDynamicLinkIdEndpoint, WRITE_RATE_LIMIT } from "./_shared.ts";

export class AdminDynamicLinkDeleteEndpoint extends AdminDynamicLinkIdEndpoint {
  protected rateLimit() {
  ...
  protected async run(_ctx: ApiContext): Promise<Response> {
    ...
    const result = await clients.devops.dynamicLinks.link.remove(this.id);
