...
import { AdminDynamicLinkEndpoint, expiresAtOrInvalid, payload, payloadOrNull, WRITE_RATE_LIMIT } from "./_shared.ts";

export class AdminDynamicLinkCreateEndpoint extends AdminDynamicLinkEndpoint {
  protected rateLimit() {
  ...
  protected async run(ctx: ApiContext): Promise<Response> {
    ...
    const result = await clients.devops.dynamicLinks.link.add({
      payload: linkPayload,
      expiresAt,
    });
