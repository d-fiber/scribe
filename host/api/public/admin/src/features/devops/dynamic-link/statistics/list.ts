...
import { AdminDynamicLinkStatisticEndpoint, payload, READ_RATE_LIMIT } from "./_shared.ts";

const DEFAULT_PAGE_SIZE = 30;
const MAX_PAGE_SIZE = 100;

export class AdminDynamicLinkStatisticsListEndpoint extends AdminDynamicLinkStatisticEndpoint {
  protected rateLimit() {
  ...
  protected async run(ctx: ApiContext): Promise<Response> {
    ...
    const link = await clients.devops.dynamicLinks.link.getById(this.id);
    if (!link.ok) return this.response.notFound();
    ...
    const result = await clients.devops.dynamicLinks.statistics.pagination(this.id, { offset, size });
    if (!result.ok) return this.notFoundOrUnexpected(result.error);

    const page = result.data;
    return this.response.ok({
      data: { statistics: page.items.map(payload), pagination: page.pagination },
    });
  }
}
