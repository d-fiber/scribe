...
import { AdminDynamicLinkEndpoint, payload, READ_RATE_LIMIT } from "./_shared.ts";

const DEFAULT_PAGE_SIZE = 30;
const MAX_PAGE_SIZE = 100;

export class AdminDynamicLinkListEndpoint extends AdminDynamicLinkEndpoint {
  protected rateLimit() {
  ...
  protected async run(ctx: ApiContext): Promise<Response> {
    ...
    const result = await clients.devops.dynamicLinks.link.pagination({ offset, size });
    if (!result.ok) return this.failure(result.error);

    const page = result.data;
    return this.response.ok({
      data: { links: page.items.map(payload), pagination: page.pagination },
    });
  }
}
