...
import { VerifyOtpError } from "@scribe/host/dependencies/security/auth/src/sign_in/sign_in.ts";
...
export class VerifyOtpEndpoint extends ApiEndpoint {
  ...
  protected async run(ctx: ApiContext): Promise<Response> {
    ...
    const result = await clients.security.auth.signIn.admin.email.verifyOtp(body.token, body.otp);

    if (!result.ok) {
      switch (result.error) {
        case VerifyOtpError.InvalidOrExpired:
          return this.response.badRequest({
            message:
              "The verification code you entered is invalid or has expired. Please check the code and try again.",
          });
        case VerifyOtpError.TooManyRequests:
    ...
    if (result.data.app_metadata.role !== AccountRole.Admin) {
