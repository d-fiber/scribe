...
export class PermissionsEndpoint extends ApiEndpoint {
  ...
  protected async run(ctx: ApiContext): Promise<Response> {
    ...
    const permissions = (await RbacIdentity.permissions()).filter(
      (p) => !p.startsWith("intra:"),
    );

    return this.response.ok({ data: permissions });
  }
}
