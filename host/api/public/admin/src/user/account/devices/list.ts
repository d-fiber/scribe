...
import { DevicesError } from "@scribe/host/dependencies/security/auth/src/session/session.ts";
import { Time } from "@scribe/core/contracts/common/time.ts";
import { ApiContext, ApiEndpoint, Caller } from "@scribe/core/kernel/endpoint/api.ts";

export class ListDevicesEndpoint extends ApiEndpoint {
  ...
  protected async run(_ctx: ApiContext): Promise<Response> {
    const result = await clients.security.auth.session.device.devices();

    if (!result.ok) {
      switch (result.error) {
        case DevicesError.Unauthorized:
