...
import { handleIntraAuth } from "@scribe/host/api/internal/auth/intra/endpoint.ts";

class Intra extends Router {
  protected routes(app: Hono): void {
    app.all("*", () => handleIntraAuth());
    app.all("*", () => ServerResponse.notFound());
  }
}

export const app = Intra.create();

if (import.meta.main) serveFunction(app, "intra");
