...
export class CreateEndpoint extends ServiceEndpoint {
  protected async run(ctx: ApiContext): Promise<Response> {
    ...
    const sender = await clients.features.messagings.mail.for(body.account);
    if (!sender.ok) return this.response.badRequest();

    await sender.data.create(
      body.to,
      body.template_name,
