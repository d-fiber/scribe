...
import { PushSendEndpoint } from "@scribe/host/api/internal/messaging/push/src/send.ts";

class Push extends Router {
  protected routes(app: Hono): void {
    app.post("/send", () => PushSendEndpoint.handle());
    app.all("*", () => ServerResponse.notFound());
  }
}

export const app = Push.create();
