...
export class DeleteVpnEndpoint extends ServiceEndpoint {
  protected async run(context: ApiContext): Promise<Response> {
    ...
    const deleted = await clients.security.vpn.deleteAll(body.admin_id);
    if (!deleted.ok) return this.response.unexpected();
