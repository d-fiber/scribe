...
import { RateLimitScope } from "./contract.ts";
...
function currentCaller():
  | (Omit<ResolvedCaller, "key"> & { id: string })
  | null {
  try {
    ...
    const ip = request.ip();
    return ip.length > 0 ? { id: ip, kind: CallerKind.Address } : null;
  } catch {
    return null;
  }
}
...
export function resolveCaller(
  ...
  if (scope === RateLimitScope.Global) {
    return { key, kind: CallerKind.Global };
  }

  const caller = currentCaller();
  if (caller === null) return null;

  return { key: `${key}:${caller.id}`, kind: caller.kind };
}
