...
// `kv.ts` reads `Env.REDIS_URL` at module load time (the `ioredis` client
// itself is `lazyConnect: true`, so no real connection is attempted but the
// env var must exist). Use `tests/.env.test` (see `.claude/testing.md`) or
// set `REDIS_URL` yourself before importing this file.
...
// graph — `Valkery` subclasses aren't imported directly here.
